...
All teams (6–8 members) implement the full system as described, including all workers, replication across three or more services, and poison pill / dead letter handling on all queues.

Every team must include: at least one synchronous service-to-service call, at least one async pipeline using Redis pub/sub or queues, at least one Redis cache, database-per-service isolation, idempotency on at least one write path, and a poison pill handling strategy on every queue.

Each team member should own either a **complete service** (its routes, database schema, Redis interactions, and Dockerfile) or a **vertical slice** (e.g., the full export pipeline: the API endpoint that enqueues the job, the worker that processes it, and the status endpoint that reports results). Ownership must be declared in each sprint plan and backed by commit history.
...
| **Refund Service**          | service                 | Handles refund requests. Validates the purchase exists via a synchronous call to the Ticket Purchase Service, then calls the Payment Service to reverse the charge and publishes a "seat released" event on Redis pub/sub so the Waitlist Worker can promote the next user. Idempotent — a duplicate refund request for the same purchase must not double-reverse. Owns its own refund database. |
| **Fraud Detection Worker**  | worker                  | Consumes purchase events from a Redis queue. Checks for suspicious patterns (same payment token used across multiple events in a short window, rapid sequential purchases from the same user). Flags suspicious purchases by writing to a fraud database and publishing a "fraud flagged" event on pub/sub. Handles poison pills (malformed payment records). |
...
| **Refund DB**               | database                | Stores refund records and reversal status. Owned exclusively by the Refund Service.                                                                                                                                                                                                                      |
| **Fraud DB**                | database                | Stores fraud flags and pattern records. Owned exclusively by the Fraud Detection Worker.                                                                                                                                                                                                                 |
| **Redis**                   | cache + queue + pub/sub | Caches event details. Provides the waitlist queue, the analytics event queue, the fraud detection queue, and the purchase confirmation pub/sub channel.                                                                                                                                                  |
...
| **Rating DB**                  | database                | Stores customer ratings and restaurant aggregate scores. Owned by the Rating & Review Service.                                                                                                                                                                     |
| **Pricing DB**                 | database                | Stores surge periods and multipliers. Owned by the Surge Pricing Worker.                                                                                                                                                                                           |
| **Redis**                      | cache + queue + pub/sub | Caches menus. Provides the order dispatch queue, notification queue, order volume queue, and pub/sub channels for order lifecycle and rating events.                                                                                                                |
...
| **Moderation Worker**   | worker                  | Listens on Redis pub/sub for "transcode complete" events. Simulates content review (configurable pass rate). Writes the moderation result to the moderation database. If rejected, publishes a "video rejected" event so the Catalog Service can mark the video as unavailable. Must handle poison pills (malformed transcode records). |
| **Playback Service**    | service                 | Tracks video view events submitted by clients. Writes view records idempotently to its own playback database — a duplicate view event for the same user and video within 30 seconds is ignored. Exposes a "resume position" endpoint returning the last watched position. Publishes "view started" events on Redis pub/sub so the Search Index Worker can boost relevance scores. |
...
| **Moderation DB**       | database                | Stores moderation results and flags. Owned by the Moderation Worker.                                                                                                                                                                                                                                       |
| **Playback DB**         | database                | Stores view records and resume positions. Owned by the Playback Service.                                                                                                                                                                                                                                   |
| **Redis**               | cache + queue + pub/sub | Caches video metadata. Provides the transcode job queue and pub/sub channels for pipeline and view events.                                                                                                                                                                                                 |
...
| **Report Generation Worker** | worker                  | Consumes "report requested" events from a Redis queue. Reads from the Readings DB to compute time-range summaries (min/max/average per sensor). Writes completed reports to the report database and publishes a "report ready" event on pub/sub. Idempotent — a duplicate request for the same sensor and time range must not produce a second report. Handles poison pills (requests for sensors with no data). |
| **Device Management Service**| service                 | Manages sensor registration, firmware versions, and maintenance schedules. Called synchronously by the Sensor Registry Service to validate that a sensor ID is known before its config is cached. Owns the device database. Exposes endpoints to register new sensors and record firmware updates. Publishes "device registered" events on Redis pub/sub so the Ingestion Service can warm its validation cache. |
...
| **Report DB**                | database                | Stores generated summary reports. Owned by the Report Generation Worker.                                                                                                                                                |
| **Device DB**                | database                | Stores sensor registration records, firmware versions, and maintenance schedules. Owned by the Device Management Service.                                                                                               |
| **Redis**                    | cache + queue + pub/sub | Caches latest readings and sensor configs. Provides the ingestion queue, report request queue, and pub/sub channels for alerts and device registration events.                                                          |
...
| **Comment Service**     | service                 | Manages comments and threaded replies on documents. Calls the Auth Service synchronously to verify the commenter has at least read permission on the document. Owns the comment database. Publishes "comment added" events on Redis pub/sub so the Notification Worker can alert collaborators.                                                                                  |
| **Activity Feed Worker**| worker                  | Consumes "document updated", "comment added", and "export completed" events from Redis pub/sub. Builds a per-user activity feed by writing denormalized events to the activity database, ordered by timestamp. Exposes a read endpoint for the dashboard to query recent activity for a given user. Tolerates duplicate events via idempotent upserts keyed on event ID and user ID. |
...
| **Comment DB**          | database                | Stores comments and reply threads. Owned by the Comment Service.                                                                                                                                                                                                                                         |
| **Activity DB**         | database                | Stores per-user activity feed entries. Owned by the Activity Feed Worker.                                                                                                                                                                                                                                |
| **Redis**               | cache + queue + pub/sub | Caches documents and permissions. Provides the export job queue and pub/sub channels for document and comment events.                                                                                                                                                                                    |
