...
Submit the following files to the LMS. You will produce these as you work through the three scenarios below. Do not include the JSON summary files generated by k6 (`k6-summary*.json`).

**Scenario 1** (from `scenario-01/results/`):

- `k6-output.txt` — k6 load test output
- `notes.md` — your observations and reflection

**Scenario 2** (from `scenario-02/results/`):

- `k6-output.txt` — k6 load test output (3 replicas)
- `service-1-stats.json`, `service-2-stats.json`, `service-3-stats.json` — per-replica request counters
- `k6-output-r5.txt` through `k6-output-r30.txt` — scaling experiment k6 output for each replica count
- `scaling-results.csv` — completed CSV with p(95) values for each replica count
- `scaling-results.png` — graph generated from the CSV
- `notes.md` — your observations, comparison with Scenario 1, and scaling experiment reflection

**Scenario 3** (from `scenario-03/results/`):

- `k6-output.txt` — k6 stress test output
- `service-1-stats.json`, `service-2-stats.json`, `service-3-stats.json` — post-test replica stats
- `notes.md` — your observations, threshold results, and SLO reflection
...
- **Scenario 2** (3 replicas + Caddy): ~6 req/s, roughly 1/3 the latency of Scenario 1
- **Scenario 3** (3 replicas + Caddy, ramping to 800 VUs): the system holds up at first, then degrades and eventually breaks
...
![Scenario 2 architecture: k6 sending traffic through Caddy, which distributes to three replicas](diagrams/scenario-02-architecture.svg)
...
- `scenario-02/docker-compose.yml`: starts three backend service containers and one Caddy container
- `scenario-02/app/server.js`: blocks the event loop for 500ms per request, tracks per-container request count
- `scenario-02/app/Dockerfile`: builds the service image used by all three backends
- `scenario-02/Caddyfile`: tells Caddy to distribute requests across the three backends with round robin
...
- `scenario-02/load-test.js`: sends load to Caddy on port `80` with no pauses between requests
...
Unlike the previous activity where all traffic went to one replica, you should now see work showing up on multiple replicas.
...
These files will help you compare how traffic was distributed across replicas. The request counts should be roughly equal across all three.

### Step 7: Compare your results

Compare this scenario against Scenario 1 and write down:

- requests/sec -- this should be roughly 3x what you saw in Scenario 1
- p95 latency -- this should be roughly 1/3 of what you saw earlier
- error rate
- what evidence you see that requests are being distributed
- how the request counters compare across replicas
...
- How do your results compare with Scenario 1? You should now see a clear, measurable difference.
...
This reads `results/scaling-results.csv` and writes `results/scaling-results.png`. Because the results directory is bind-mounted, the PNG will appear in `scenario-02/results/` on your machine. Open it to see the relationship between replica count and p(95) latency.
...
Scenarios 1 and 2 showed that load balancing helps. But every system has a ceiling. In this scenario, you will ramp traffic until the load-balanced system cannot keep up. The goal is to find out where the system breaks and what that looks like in the data.
...
This scenario uses the same 3-replica + Caddy architecture as Scenario 2, but with two important changes:
...
- `scenario-03/docker-compose.yml`: starts three resource-constrained backend service containers and one Caddy container
- `scenario-03/app/server.js`: blocks the event loop for 500ms per request, tracks per-container request count and memory usage
...
- `scenario-03/load-test.js`: ramps from 100 to 800 VUs with pass/fail thresholds
...
Run it a few times. The system should respond normally, just like Scenario 2.
...
Compare this scenario against Scenario 2 and write down:
